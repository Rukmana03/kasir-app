  ...
  const handleUpdatePayment = async (idTransaksi, newMethod) => {
    if (window.confirm(`Ubah metode pembayaran menjadi ${newMethod.toUpperCase()}?`)) {
      try {
        ...
        await axios.patch(`\${import.meta.env.VITE_API_BASE_URL}/transaksi/${idTransaksi}/payment`, {
    ...
    <div className="container-fluid py-4 fade-in" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <div className="d-flex align-items-center gap-2 bg-white px-3 py-2 rounded-pill shadow-sm border">
          <label className="text-muted small fw-semibold mb-0"><i className="fas fa-calendar-alt me-2"></i>Tanggal:</label>
          <input 
            type="date" 
      ...
      <div className="card border-0 shadow-sm rounded-4 section-card overflow-hidden">
        <div className="card-header bg-white border-bottom py-3 px-4">
          <h5 className="fw-bold mb-0 text-dark"><i className="fas fa-receipt me-2 text-primary"></i>Daftar Nota</h5>
        </div>
        <div className="card-body p-0">
          ...
          {!loading && transactions.length > 0 && transactions.map((tx, index) => (
                  ...
                  <div className="mb-1">
                    {(!tx.id_meja || tx.jenis_transaksi === 'Takeaway') ? (
                       ...
                       <span className="fw-semibold text-dark me-2">
                         <i className="fas fa-chair text-muted me-1"></i>{tx.meja?.nama_meja || tx.nama_meja || `Meja ${tx.id_meja}`}
                       </span>
                    )}
                    {tx.nama_pelanggan && (
                      <span className="text-muted small">• Pelanggan: <strong className="text-dark">{tx.nama_pelanggan}</strong></span>
                    )}
                  </div>
                  
                  {/* Item List */}
                  <div className="small text-muted mt-2">
                    {tx.detail_transaksi && tx.detail_transaksi.map((it, idx) => (
                      <div key={idx}>
