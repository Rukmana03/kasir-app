...
export default function KalkulatorBelanja() {
  ...
  const handleHitung = async () => {
    ...
    try {
      ...
      const dataDenganKemasan = res.data.data.map(item => {
            ...
            <div className="d-flex flex-wrap gap-2">
              {targetItems.map(item => (
                <span key={item.id_menu} className="badge bg-white text-dark border p-2 d-flex align-items-center">
                  <span className="fw-bold me-2">{item.nama_menu} ({item.porsi} porsi)</span>
            ...
            <div className="table-responsive">
              <table className="table align-middle mb-0">
                ...
                <thead className="table-light text-center">
                  <tr>
                    <th className="text-start" style={{ width: '25%' }}>Bahan Baku</th>
                    <th style={{ width: '15%' }}>Wajib Beli</th>
                    <th style={{ width: '40%' }}>Input Harga Pasar & Takaran</th>
                    <th className="text-end" style={{ width: '20%' }}>Estimasi Biaya</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {hasilKalkulasi.map((h, index) => (
                    <tr key={index}>
                      <td className="fw-semibold text-start">
                        {h.nama_bahan} <br/>
                        <small className="text-muted fw-normal">Sisa: {h.sisa_stok} {h.satuan}</small>
                      </td>
                      ...
                      <td className="text-center text-danger fw-bold fs-6">
                        {h.harus_beli > 0 ? (
                          ...
                          <span className="text-success fs-6"><i className="fas fa-check me-1"></i> Aman</span>
                        )}
                      </td>
                        ...
                        {h.harus_beli > 0 ? (
                            <div className="d-flex align-items-center justify-content-center gap-2">
                                ...
                                <div className="input-group input-group-sm shadow-sm" style={{ maxWidth: '140px' }}>
                                    ...
                                    <input 
                                        type="number" 
                                        className="form-control text-end border-start-0 ps-0" 
                                        value={h.harga_kemasan || ''} 
                                        onChange={(e) => handleUbahPasar(index, 'harga_kemasan', e.target.value)}
                                        placeholder="Harga"
                                    />
                                </div>
                                ...
                                <div className="input-group input-group-sm shadow-sm" style={{ maxWidth: '120px' }}>
                                    <input 
                                        type="number" 
                                        className="form-control text-center border-end-0 pe-0" 
                                        value={h.isi_kemasan || ''} 
                                        onChange={(e) => handleUbahPasar(index, 'isi_kemasan', e.target.value)}
                                        placeholder="Takaran"
                                    />
                      ...
                      <td className="text-end fw-bold text-primary">
                        {h.harus_beli > 0 ? (
                            `Rp ${Math.round(h.estimasi_biaya_total || 0).toLocaleString('id-ID')}`
                        ) : (
                            'Rp 0'
                        )}
                      </td>
                  ...
                  <tr className="table-primary border-primary">
                    <td colSpan="3" className="text-end fw-bold fs-6">TOTAL ESTIMASI ANGGARAN:</td>
                    <td className="text-end fw-bold fs-5 text-primary">
                      Rp {Math.round(hasilKalkulasi.reduce((total, item) => total + (item.estimasi_biaya_total || 0), 0)).toLocaleString('id-ID')}
                    </td>
                  </tr>
                </tbody>
              </table>
