...
export default function BelanjaIndex() {
  ...
  const fetchMasterBahan = async () => {
    try {
      ...
      console.error("Gagal memuat master bahan");
    }
  };
  ...
  const fetchDrafRekomendasi = async () => {
    try {
      ...
      console.error("Gagal memuat draf rekomendasi");
    }
  };
  ...
  const handleBelanja = async (e) => {
    ...
    try {
      await axios.post(
        import.meta.env.VITE_API_BASE_URL + "/belanja_bahan",
        {
      ...
      showAlert("danger", "Gagal mencatat pembelanjaan.");
    } finally {
  ...
  const handlePengambilan = async (e) => {
    ...
    try {
      await axios.post(
        import.meta.env.VITE_API_BASE_URL + "/penggunaan_stok",
        {
      ...
      showAlert("danger", "Gagal mencatat pengambilan.");
    } finally {
      ...
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div>
          <h1 className="h4 fw-bold text-primary mb-1">
            <i className="fas fa-boxes-stacked me-2"></i>Belanja & Restock
          </h1>
          ...
          <div className="card border-0 shadow-sm rounded-4 mb-3" style={{ transition: "transform .25s ease" }}>
            <div className="card-body p-4 bg-primary-subtle rounded-4" style={{ backgroundColor: "rgba(13,110,253,.05)" }}>
              ...
              <div className="d-flex gap-2 mb-3">
                <input type="date" className="form-control fw-semibold" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} />
                  ...
                  {rekomendasi.map((r) => {
                    ...
                    return (
                      <span key={r.id_bahan} className="badge bg-danger text-white p-2 border border-danger text-start lh-base">
                        ...
                        {namaBahan} <br />
                        <small className="fw-normal">({pesan})</small>
                      </span>
                    );
                  })}
                </div>
                ...
                <div className="col-md-6">
                  ...
                  <div className="input-group input-group-sm">
                    <span className="input-group-text bg-light">
                      <i className="fas fa-boxes-stacked text-muted"></i>
                    </span>
              ...
              <form onSubmit={handlePengambilan} className="row g-3">
                <div className="col-md-7">
                  ...
                  <select className="form-select form-select-sm" value={formPengambilan.id_bahan} onChange={(e) => setFormPengambilan({ ...formPengambilan, id_bahan: e.target.value })} required>
                    ...
                    {bahan.map((b) => (
                      <option key={b.id_bahan} value={b.id_bahan}>
                        {b.nama_bahan} (Stok: {b.stok || 0} {b.satuan})
                      </option>
                    ))}
                  </select>
                </div>
                      ...
                      bahan.map((b) => {
                        ...
                        return (
                          <tr key={b.id_bahan}>
                            ...
                            <td>
                              <span className={`badge ${b.jenis_bahan === "kering" ? "bg-warning" : "bg-info"} text-dark border`}>{b.jenis_bahan}</span>
                            </td>
                        ...
                        {logBelanja.length === 0 ? (
                          <tr>
                            <td colSpan="3" className="text-center text-muted py-4 small">
                              <i className="fas fa-box-open mb-2 d-block fs-3"></i>Kosong
                            </td>
                          </tr>
                        ) : (
                        ...
                        {logPengambilan.length === 0 ? (
                          ...
                          logPengambilan.map((log, i) => (
                            <tr key={i}>
                              ...
                              <td className="small text-muted text-truncate text-end" style={{ maxWidth: "80px" }} title={log.keterangan}>
                                {log.keterangan || "-"}
                              </td>
                            </tr>
                          ))
