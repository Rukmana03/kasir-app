...
export default function MenuIndex() {
  ...
  const fetchMenus = async () => {
    ...
    } catch (error) { showAlert('danger', 'Gagal mengambil data menu.'); } 
  ...
  const fetchBahanMaster = async () => {
    ...
    } catch (error) { console.error("Gagal load bahan master"); }
  };
  ...
  const handleToggleStatus = async (menu) => {
    try {
      ...
      fetchMenus(); 
    } catch (error) { showAlert('danger', 'Gagal mengubah status menu.'); }
  };

  const confirmDelete = (menu) => { setMenuToDelete(menu); setShowDeleteModal(true); };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setShowDeleteModal(false); fetchMenus();
    } catch (error) { showAlert('danger', 'Tidak dapat dihapus karena terhubung transaksi.'); } 
  ...
  const handleTambahResep = async (e) => {
    ...
    try {
      await axios.post(`\${import.meta.env.VITE_API_BASE_URL}/menu/${activeMenuResep.id_menu}/resep`, {
  ...
  const processedMenus = menus.filter(m => {
    ...
    if (sortField === 'harga') { aVal = Number(aVal); bVal = Number(bVal); }
    ...
    <div className="container-fluid px-0 fade-in">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <Link to="/menu/create" className="btn btn-primary rounded-pill d-flex align-items-center">
          <i className="fas fa-plus me-2"></i><span className="d-none d-md-inline">Tambah Menu</span>
        </Link>
      </div>
            ...
            <div className="row g-3 align-items-center">
              <div className="col-12 col-md-6">
                <div className="input-group input-group-lg">
                  ...
                  {search && <button className="btn btn-outline-secondary bg-white border-start-0" onClick={() => setSearch('')}><i className="fas fa-times"></i></button>}
                </div>
              </div>
              <div className="col-12 col-md-6">
          ...
          <div className="table-responsive p-4">
            <table className="table align-middle table-hover mb-0">
              <thead className="table-light">
                <tr>
                  <th className="text-center user-select-none" style={{ width: '80px', cursor: 'pointer' }} onClick={() => handleSort('id_menu')}>ID {getSortIcon('id_menu')}</th>
                  <th className="user-select-none" style={{ cursor: 'pointer' }} onClick={() => handleSort('nama_menu')}>Nama Menu {getSortIcon('nama_menu')}</th>
                  <th style={{ width: '160px' }}>Kategori</th>
                  <th className="text-end user-select-none" style={{ width: '140px', cursor: 'pointer' }} onClick={() => handleSort('harga')}>Harga {getSortIcon('harga')}</th>
              ...
              <tbody>
                {isLoading ? (
                  <tr><td colSpan="6" className="text-center py-5"><i className="fas fa-spinner fa-spin fa-2x text-primary"></i></td></tr>
                ) : processedMenus.length === 0 ? (
                  <tr><td colSpan="6" className="text-center text-muted py-5"><i className="fas fa-search fa-3x mb-3 text-secondary"></i><p>Menu tidak ditemukan</p></td></tr>
                ) : (
                  processedMenus.map(r => (
                    <tr key={r.id_menu}>
                      ...
                      <td><span className="badge bg-info text-dark rounded-pill">{r.kategori}</span></td>
                      <td className="text-end fw-semibold">Rp {r.harga.toLocaleString('id-ID')}</td>
                      <td>
                        {r.status_menu === 'tersedia' ? <span className="badge bg-success rounded-pill"><i className="fas fa-check-circle me-1"></i> Aktif</span> : <span className="badge bg-secondary rounded-pill"><i className="fas fa-times-circle me-1"></i> Tidak Aktif</span>}
                      </td>
                      <td className="text-end">
                        <div className="btn-group">
                          <Link to={`/menu/edit/${r.id_menu}`} className="btn btn-sm btn-outline-primary rounded-start-pill" title="Edit Data Dasar"><i className="fas fa-pen"></i></Link>
                          ...
                          <ul className="dropdown-menu dropdown-menu-end shadow-sm">
                            ...
                            <li>
                              <button className="dropdown-item" onClick={() => handleToggleStatus(r)}>
                                <i className="fas fa-power-off me-2"></i>{r.status_menu === 'tersedia' ? 'Nonaktifkan Menu' : 'Aktifkan Menu'}
                              </button>
                            </li>
                            <li>
          ...
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content border-0 shadow-lg rounded-4 slide-up">
              <div className="modal-header border-bottom-0 pb-0 pt-4 px-4">
                <div>
                  <h5 className="modal-title fw-bold text-primary"><i className="fas fa-clipboard-list me-2"></i>Resep Menu</h5>
                  <p className="text-muted small mb-0">Menu: <strong>{activeMenuResep?.nama_menu}</strong></p>
                </div>
              ...
              <div className="modal-body p-4">
                ...
                <form onSubmit={handleTambahResep} className="row g-2 mb-4 bg-light p-3 rounded-3 border">
                  ...
                  <div className="col-md-3 d-flex align-items-end">
                    <button type="submit" className="btn btn-primary btn-sm w-100 fw-bold" disabled={isSavingResep}>
                      {isSavingResep ? 'Menyimpan...' : <><i className="fas fa-plus me-1"></i> Tambah</>}
                    </button>
                  </div>
                </form>
                ...
                <div className="table-responsive" style={{ maxHeight: '30vh', overflow: 'auto' }}>
                  <table className="table table-sm table-hover align-middle">
                    ...
                    <tbody>
                      {resepList.length === 0 ? (
                        <tr><td colSpan="3" className="text-center text-muted py-4 small">Belum ada bahan yang ditambahkan ke resep ini.</td></tr>
                      ) : (
                        resepList.map((resep) => (
              ...
              <div className="modal-footer border-0 pt-0 px-4 pb-4">
                <button type="button" className="btn btn-light rounded-pill px-4 w-100" onClick={closeResepModal}>Selesai & Tutup</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow">
              <div className="modal-header bg-danger text-white">
                <h5 className="modal-title"><i className="fas fa-triangle-exclamation me-2"></i>Konfirmasi Hapus</h5>
              ...
              <div className="modal-footer justify-content-center border-0">
                <button type="button" className="btn btn-light rounded-pill px-4" onClick={() => setShowDeleteModal(false)}>Batal</button>
                <button type="button" className="btn btn-danger rounded-pill px-4" onClick={handleDelete} disabled={isDeleting}>
                  {isDeleting ? 'Menghapus...' : <><i className="fas fa-trash me-1"></i> Ya, Hapus</>}
                </button>
              </div>
            </div>
          </div>
