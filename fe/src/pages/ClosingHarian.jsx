        ...
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              ...
              <div className="text-muted d-flex align-items-center mt-1">
                ...
                {isAdmin ? <input type="date" className="border-0 bg-transparent text-muted p-0 fw-semibold" style={{ outline: "none", cursor: "pointer" }} value={date} onChange={(e) => setDate(e.target.value)} /> : <span className="fw-semibold">{formatDate(date)}</span>}
              </div>
            </div>
          </div>
          ...
          <div className="d-flex flex-wrap gap-2">
            {!isAdmin && isClosed && isToday && (
              <button onClick={handleReopen} disabled={isSubmitting} className="btn btn-warning rounded-pill shadow-sm fw-semibold">
            ...
            {loading ? (
              ...
              <div className="text-center py-4">
                <div className="p-4 text-muted">
                  <i className="fas fa-circle-info fa-2x mb-3 opacity-50"></i>
                  <br />
                  <h5 className="fw-bold text-dark mb-1">Belum ada closing untuk tanggal ini.</h5>
                  <p className="small text-muted mb-0">
                    Terdapat {unclosedTxCount} transaksi dengan total Rp {formatRp(unclosedSalesTotal)} yang belum dikunci.
                  </p>
                </div>
                <div className="mt-2 d-inline-block">
                  {!isAdmin && (
                    <button onClick={handleBuatClosing} disabled={isSubmitting} className="btn btn-primary btn-lg rounded-pill fw-bold shadow-sm px-4">
                      {isSubmitting ? (
                        "Memproses..."
                      ) : (
                        <>
                          <i className="fas fa-flag-checkered me-2"></i>Buat Closing Sekarang
                        </>
                      )}
                    </button>
                  )}
              ...
              <>
                ...
                <h2 className="h6 fw-bold mb-3">
                  <i className="fas fa-chart-pie me-2 text-primary"></i>Ringkasan
                </h2>
                <div className="row g-3 mb-5">
                  ...
                  <div className="col-6 col-md-3">
                    <div className="mini-card border-0 shadow-sm h-100 section-card">
                      <div className="mini-card-body">
                        <div className="mini-icon text-info">
                  ...
                  <div className="col-6 col-md-3">
                    <div className="mini-card border-0 shadow-sm h-100 section-card">
                      <div className="mini-card-body">
                        <div className="mini-icon text-warning">
                ...
                {closingData.byMethod && closingData.byMethod.length > 0 && (
                  <div className="subcard mb-5">
                    <div className="d-flex align-items-center mb-3">
                      <div className="icon-circle bg-info-subtle text-info me-2" style={{ width: "36px", height: "36px", fontSize: "15px" }}>
                      ...
                      <table className="table table-hover table-sm align-middle mb-0">
                        ...
                        <tbody>
                          {closingData.byMethod.map((m, idx) => (
                            <tr key={idx}>
                              <td className="ps-3">
                                <span className={`badge ${m.method === "cash" ? "bg-success" : "bg-primary"}`}>{m.method.toUpperCase()}</span>
                              </td>
                ...
                {closingData.wet_spend && Object.keys(closingData.wet_spend).length > 0 && (
                  <div className="subcard mb-5">
                    <div className="d-flex align-items-center mb-3">
                      <div className="icon-circle bg-primary-subtle text-primary me-2" style={{ width: "36px", height: "36px", fontSize: "15px" }}>
                        <i className="fas fa-water"></i>
                      </div>
                      ...
                      <table className="table table-hover table-sm mb-0">
                        <thead className="table-light">
                          <tr>
                            ...
                            <th className="text-end pe-3 py-2">Biaya</th>
                          </tr>
                        </thead>
                        <tbody>
                          {Object.entries(closingData.wet_spend).map(([name, cost], idx) => (
                            <tr key={idx}>
                              ...
                              <td className="text-end pe-3">Rp {formatRp(cost)}</td>
                            </tr>
                          ))}
                          <tr className="fw-bold table-light">
                ...
                <div className="subcard">
                  <div className="d-flex align-items-center mb-3">
                    <div className="icon-circle bg-success-subtle text-success me-2" style={{ width: "36px", height: "36px", fontSize: "15px" }}>
                  ...
                  <div className="table-responsive table-scroll border rounded">
                    <table className="table table-hover table-sm align-middle mb-0">
                      <thead className="table-light sticky-top">
                        <tr>
                          <th className="ps-3 py-2">Item</th>
                          <th className="text-end pe-3 py-2" style={{ width: "120px" }}>
                            Qty
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {closingData.items_sold && Object.keys(closingData.items_sold).length > 0 ? (
                          Object.entries(closingData.items_sold).map(([name, qty], idx) => (
                            <tr key={idx}>
                              <td className="ps-3 fw-semibold text-secondary">{name}</td>
                              <td className="text-end pe-3">
                                <span className="badge bg-light border text-dark">{qty}</span>
                              </td>
                            </tr>
                          ))
                        ) : (
      ...
      {isClosed && closingData && (
        <div className="print-only">
          <div className="print-header">
            <h1>
              {isAdmin ? "Laporan Harian" : "Closing Harian"} — {formatDate(date)}
            </h1>
          </div>
            ...
            <table className="print-table mb-4">
              <tbody>
                ...
                <tr>
                  ...
                  <td>{closingData.transactions_count}</td>
                </tr>
                <tr>
                  <th>Pemasukan</th>
                  <td>Rp {formatRp(closingData.sales_total)}</td>
                </tr>
                <tr>
                  <th>Belanja</th>
                  <td>Rp {formatRp(closingData.wet_spend_total)}</td>
                </tr>
                <tr>
                  <th>Total Bersih</th>
                  <td>
                    <strong>Rp {formatRp(closingData.sales_total - (closingData.wet_spend_total || 0))}</strong>
                  </td>
                </tr>
              </tbody>
            </table>
                    ...
                    {closingData.byMethod.map((m, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="text-end">
                          <strong>Rp {formatRp(m.total)}</strong>
                        </td>
                      </tr>
                    ))}
                    <tr className="fw-bold" style={{ backgroundColor: "#fafafa" }}>
                      <td>Total Harian</td>
                      <td className="text-end">{closingData.transactions_count}</td>
                      <td className="text-end">
                        <strong>Rp {formatRp(closingData.sales_total)}</strong>
                      </td>
                    </tr>
                  </tbody>
                </table>
                ...
                <table className="print-table mb-4">
                  ...
                  <tbody>
                    {Object.entries(closingData.wet_spend).map(([name, cost], idx) => (
                      <tr key={idx}>
                        <td>{name}</td>
                        <td className="text-end">Rp {formatRp(cost)}</td>
                      </tr>
                    ))}
                    <tr className="fw-bold" style={{ backgroundColor: "#fafafa" }}>
                      <td>Total Belanja</td>
                      <td className="text-end">
                        <strong>Rp {formatRp(closingData.wet_spend_total)}</strong>
                      </td>
                    </tr>
                  </tbody>
                </table>
